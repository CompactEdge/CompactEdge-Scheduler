...
class KubernetesMonitoringService():
    ...
    def __init__(self):
        self.MONITORING_URL = "http://{0}:{1}".format(
            current_app.config["MONITORING_IP"], str(current_app.config["MONITORING_PORT"]))
    ...
    def find_by_monitoring_query_range(self, args):
        ...
        try:
            ...
            for expr_item in expr_items:
                ...
                monitoring_url = "{0}/api/v1/query_range".format(
                    self.MONITORING_URL)
                monitoring_params = {
                    "start": start_time,
                    "end": end_time,
                ...
                response = requests.get(
                    url=monitoring_url, params=monitoring_params)
                ...
                if len(result_data["data"].keys()) != 0:
                    ...
                    result_data["data"]["result"] = result_data["data"]["result"] + \
                ...
                for x in response_data["data"]["result"]:
                    ...
                    for index in range(len(values)):
                        values[index][0] = str(datetime.datetime.fromtimestamp(
                            values[index][0]).strftime("%Y-%m-%d %H:%M:%S"))
    ...
    def find_by_monitoring_variable_values(self, variable_values_args):
        ...
        try:
            ...
            result_data["status"].append(response_data["status"])
            result_data["status_code"].append(response_status_code)
            result_data["data"] = {"resutType": "list", "result": sorted(list(set(
                data['metric'][expr_item_label] for data in response_data["data"]['result'])))}
        except requests.exceptions.RequestException as request_err:
            result_data = self.make_request_error(request_err)
        except KeyError as key_err:
            result_data = self.make_key_error(key_err)
    ...
    def find_by_monitoring_meta_datas(self, meta_datas_args):
        ...
        try:
            ...
            result_data["data"] = {
                "resutType": "list", "result": list(panel_names)}
        except KeyError as key_err:
            result_data = self.make_key_error(key_err)
